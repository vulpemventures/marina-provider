 ...
 */
export async function detectProvider<T = MarinaProvider>(provider: string = 'marina', timeout: number = 3000): Promise<T> {
  let handled = false;
...
export class DetectProviderTimeoutError extends Error {
  ...
  constructor(provider: string, timeout: number) {
    super(`detectProviderTimeout: detection of ${provider} timeout (${timeout} ms)`)
  }
}